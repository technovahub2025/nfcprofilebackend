...
// SOCIAL HELPERS
const cleanInstagram = (value = "") =>
  value ? `https://instagram.com/${value.replace("@", "").trim()}` : "";
...
exports.createProfile = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      profileJsonUrl: `/tbc/api/profile/${savedProfile._id}`,
      profilePageUrl: `/tbc/api/profile-page/${savedProfile._id}`,
      fullProfileJsonUrl: `${req.protocol}://${req.get(
        "host"
      )}/tbc/api/profile/${savedProfile._id}`,
      fullProfilePageUrl: `${req.protocol}://${req.get(
        "host"
      )}/tbc/api/profile-page/${savedProfile._id}`,
    });
  } catch (err) {
    console.error("========== CREATE PROFILE ERROR ==========");
    console.error("ERROR MESSAGE:", err.message);
...
// GET PROFILE HTML PAGE WITH DEBUG
exports.getProfileHtml = async (req, res) => {
  try {
    console.log("========== HTML PROFILE DEBUG ==========");
    console.log("PROFILE PAGE ID:", req.params.id);
...
${
  instagramUrl
    ? `
<a class="btn instagram" href="${instagramUrl}" target="_blank">
Instagram
</a>
`
    : ""
...
${
  linkedinUrl
    ? `
<a class="btn linkedin" href="${linkedinUrl}" target="_blank">
LinkedIn
</a>
`
    : ""
...
${
  facebookUrl
    ? `
<a class="btn facebook" href="${facebookUrl}" target="_blank">
Facebook
</a>
`
    : ""
...
${
  googleBusinessUrl
    ? `
<a class="btn googlebusiness" href="${googleBusinessUrl}" target="_blank">
Google Business
</a>
`
    : ""
...
${
  websiteUrl
    ? `
<a class="btn website" href="${websiteUrl}" target="_blank">
Website
</a>
`
    : ""
...
`;
    ...
    console.error("========== HTML PROFILE ERROR ==========");
    console.error("PROFILE ID:", req.params.id);
    console.error("ERROR MESSAGE:", err.message);
    console.error("ERROR STACK:", err.stack);
    console.error("========================================");
